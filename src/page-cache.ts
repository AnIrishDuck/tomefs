 ...
 * O(cached-for-file), eliminating wasteful scans when a file has many
 * cached pages but none are dirty.
 */
export class PageCache {
  ...
  async write(
    ...
    const firstNewPage =
      currentFileSize > 0 ? ((currentFileSize + PAGE_MASK) >>> PAGE_SHIFT) : 0;
   ...
   * call — atomic for IDB and one round-trip instead of two.
   *
   ...
   * has set page.dirty = true — this adds the key to the dirty indexes
   ...
   * needed (e.g., call flushAll() before clear).
   *
   * Use after closing to release the cache's memory (up to
   * maxPages * PAGE_SIZE bytes of page buffers plus the buffer pool).
   * Without clear(), these buffers stay allocated until the PageCache
