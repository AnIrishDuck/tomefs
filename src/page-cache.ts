...
/**
 * Async LRU page cache with dirty tracking.
 *
 * Same algorithm as SyncPageCache but fully asynchronous — for use with
 * async StorageBackend implementations (IDB, OPFS) outside the Emscripten
 * FS layer.
 *
  ...
  async read(
    ...
    if (lastPage > firstPage) {
      ...
      if (missingIndices.length > 1) {
        // Batch-evict space before pre-loading to reduce round-trips
