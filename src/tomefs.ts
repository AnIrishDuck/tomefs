...
export function createTomeFS(FS: any, options?: TomeFSOptions): any {
  ...
  function readPages(
    ...
    if (allCached) {
      ...
      const firstN = PAGE_SIZE - pageOffset;
      if (firstN >= toRead) {
        buffer.set(pages[firstPage].data.subarray(pageOffset, pageOffset + toRead), offset);
        return toRead;
      }
      if (pageOffset === 0) {
        buffer.set(pages[firstPage].data, offset);
      } else {
        buffer.set(pages[firstPage].data.subarray(pageOffset, PAGE_SIZE), offset);
      }
      bytesRead = firstN;
      // Full middle pages (no subarray needed)
      for (let p = firstPage + 1; p < lastPage; p++) {
        buffer.set(pages[p].data, offset + bytesRead);
        bytesRead += PAGE_SIZE;
      }
      ...
      if (firstPage < lastPage) {
        ...
        if (remaining === PAGE_SIZE) {
          buffer.set(pages[lastPage].data, offset + bytesRead);
        } else {
          buffer.set(pages[lastPage].data.subarray(0, remaining), offset + bytesRead);
        }
      }
      return toRead;
    }
  ...
  function writePages(
    ...
    if (allCached) {
      ...
      const firstN = PAGE_SIZE - pageOffset;
      {
        ...
        if (firstN >= length) {
          page.data.set(buffer.subarray(offset, offset + length), pageOffset);
          if (!page.dirty) { page.dirty = true; pageCache.addDirtyKey(page.key, sp); }
          const end = position + length;
          if (end > node.usedBytes) node.usedBytes = end;
          return length;
        }
        page.data.set(buffer.subarray(offset, offset + firstN), pageOffset);
        if (!page.dirty) { page.dirty = true; pageCache.addDirtyKey(page.key, sp); }
        bytesWritten = firstN;
      }
      // Full middle pages
      for (let p = firstPage + 1; p < lastPage; p++) {
        const page = pages[p];
        page.data.set(buffer.subarray(offset + bytesWritten, offset + bytesWritten + PAGE_SIZE), 0);
        if (!page.dirty) { page.dirty = true; pageCache.addDirtyKey(page.key, sp); }
      ...
      if (firstPage < lastPage) {
        ...
        page.data.set(buffer.subarray(offset + bytesWritten, offset + bytesWritten + remaining), 0);
        if (!page.dirty) { page.dirty = true; pageCache.addDirtyKey(page.key, sp); }
      }
      const end = position + length;
      if (end > node.usedBytes) node.usedBytes = end;
