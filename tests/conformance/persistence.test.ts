...
describe("persistence (Batch 6)", () => {
  ...
  // ------------------------------------------------------------------
  // Test 20: Write → sync → unmount → remount → read round-trips
  // ------------------------------------------------------------------

  it("file data survives unmount/remount @fast", async () => {
    // Mount, write, sync, unmount
    ...
    // Remount with same backend, read back
  ...
  it("overwritten file has new content after remount", async () => {
    ...
    // Remount, overwrite, sync, unmount again
    ...
    // Third mount — should see "replaced"
  ...
  it("deleted file does not reappear after remount", async () => {
    ...
    // Remount, delete, sync, unmount
    const { FS: FS2, tomefs: t2 } = await mountTome(backend);
    FS2.unlink(`${MOUNT}/gone`);
    syncAndUnmount(FS2, t2);

    // Third mount — file should not exist
    const { FS: FS3, Module: M3 } = await mountTome(backend);
    expect(() => FS3.stat(`${MOUNT}/gone`)).toThrow();
  });

  // ------------------------------------------------------------------
  // Test 21: Large file (multi-page) round-trips
  // ------------------------------------------------------------------
  ...
  it("partial last page is preserved correctly", async () => {
    // Write exactly PAGE_SIZE + 1 byte to verify partial page persistence
    const size = PAGE_SIZE + 1;
    const data = new Uint8Array(size);
    data[0] = 0xaa;
    data[PAGE_SIZE - 1] = 0xbb;
    data[PAGE_SIZE] = 0xcc; // first byte of second page
  ...
  // ------------------------------------------------------------------
  // Test 22: Directory tree persistence
  // ------------------------------------------------------------------
  ...
  it("removed directory does not reappear after remount", async () => {
    ...
    // Remount, delete the dir tree, sync, unmount
    const { FS: FS2, tomefs: t2 } = await mountTome(backend);
    FS2.unlink(`${MOUNT}/tempdir/f`);
    FS2.rmdir(`${MOUNT}/tempdir`);
    syncAndUnmount(FS2, t2);

    // Third mount — dir should be gone
    const { FS: FS3 } = await mountTome(backend);
  ...
  it("append-mode writes persist correctly", async () => {
    ...
    // Third mount — should see concatenated content
    const { FS: FS3 } = await mountTome(backend);
