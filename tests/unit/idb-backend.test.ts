...
describe("IdbBackend", () => {
  ...
  // -------------------------------------------------------------------
  // countPages / countPagesBatch / maxPageIndex / maxPageIndexBatch
  // -------------------------------------------------------------------
  ...
  describe("maxPageIndexBatch", () => {
    ...
    it("returns max page index for multiple files in one call", async () => {
      await backend.writePage("/a", 0, filledPage(0x01));
      await backend.writePage("/a", 5, filledPage(0x02));
      await backend.writePage("/b", 0, filledPage(0x03));

      const maxes = await backend.maxPageIndexBatch(["/a", "/b", "/missing"]);
      expect(maxes).toEqual([5, 0, -1]);
    });
  });
  ...
  // -------------------------------------------------------------------
  // renameFile (additional)
  // -------------------------------------------------------------------

  describe("renameFile (additional)", () => {
  ...
  // -------------------------------------------------------------------
  // Connection robustness (onversionchange, onblocked, onabort)
  // -------------------------------------------------------------------
