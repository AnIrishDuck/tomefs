...
describe("partial syncfs: crash during metadata write", () => {
  ...
  it("file extension with stale metadata is recovered by restoreTree @fast", async () => {
    ...
    // 3 ops), then metadata. crashAfter=3: pages succeed, crash on
    ...
    // Metadata is stale (size=2), but pages 0-2 are in the backend
  ...
  it("multiple files: partial metadata write leaves filesystem recoverable", async () => {
    ...
    // Dirty pages stay in cache. syncfs writes pages first (3 pages =
    // 3 ops), then metadata. restoreTree marks root + 3 files dirty,
    // so metaBatch = [root, alpha, beta, gamma, clean_marker].
    // crashAfter=6: 3 pages + root meta + alpha meta + beta meta succeed,
    // crash on gamma metadata.
    const crashBackend = new CrashAfterNOpsSyncBackend(inner, 6);
    const { FS: FS2, tomefs: t2 } = await mountTome(crashBackend);
    for (const name of ["alpha", "beta", "gamma"]) {
