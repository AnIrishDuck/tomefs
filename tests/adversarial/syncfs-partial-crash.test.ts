...
// ---------------------------------------------------------------------------
// Test: crash during page flush (pages written during syncfs)
// ---------------------------------------------------------------------------
...
describe("partial syncfs: crash during metadata write", () => {
  ...
  it("file extension with stale metadata is recovered by restoreTree @fast", async () => {
    ...
    // metadata. crashAfter=3: pages succeed, crash on first metadata write.
  ...
  it("multiple files: partial metadata write leaves filesystem recoverable", async () => {
    ...
    // 3 ops), then metadata (3 entries + 1 clean marker). crashAfter=5:
    // all 3 page writes succeed, 2 of 3 metadata writes succeed.
